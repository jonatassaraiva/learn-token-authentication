...
function ok(res, message, result) {
  _responde(res, 200, 'OK', message, result);
}

function created(res, message, result) {
  _responde(res, 201, 'Created.', message, result);
}

function forbidden(res, message) {
  _responde(res, 403, 'Authentication faild.', message);
}

function notFound(res, message) {
  _responde(res, 404, 'Not Found.', message);
}

function internalServerError(res, message) {
  //TODO: Log
  _responde(res, 500, 'Internal Server Error.', message);
}
